    ...
    constructor(adapters = []) {
        ...
        for (const adapter of adapters) {
            if (!ADAPTER_ID_PATTERN.test(adapter.id)) {
                throw new ArtifactError("invalid_incoming_adapter", "Incoming artifact adapter IDs must be short lowercase identifiers.");
            }
            if (ids.has(adapter.id)) {
                throw new ArtifactError("duplicate_incoming_adapter", `Incoming artifact adapter '${adapter.id}' is registered more than once.`);
            }
            ids.add(adapter.id);
        }
    ...
    async open(value) {
        ...
        for (const adapter of this.adapters) {
            ...
            try {
                ...
                throw new ArtifactError("incoming_artifact_adapter_failed", `Incoming artifact adapter '${adapter.id}' failed during recognition.`);
            }
            if (handles)
                matching.push(adapter);
        }
        if (matching.length === 0) {
            throw new ArtifactError("unsupported_incoming_artifact", "No trusted incoming artifact adapter recognized this file reference.");
        }
        if (matching.length > 1) {
            throw new ArtifactError("ambiguous_incoming_artifact", "More than one trusted incoming artifact adapter recognized this file reference.");
        }
        ...
        try {
            ...
            if (error instanceof ArtifactError)
                throw error;
            throw new ArtifactError("incoming_artifact_open_failed", `Incoming artifact adapter '${adapter.id}' could not open the file reference.`);
        }
        try {
...
export function createOpenAIIncomingArtifactAdapter(options = {}) {
    const fetchFile = options.fetch ?? globalThis.fetch;
    const timeoutMs = options.timeoutMs ?? OPENAI_FILE_DOWNLOAD_TIMEOUT_MS;
    if (!Number.isSafeInteger(timeoutMs) || timeoutMs <= 0) {
        throw new ArtifactError("invalid_openai_file_adapter", "OpenAI file download timeout must be a positive integer.");
    }
    return {
        id: "openai-file",
        ...
        async open(value) {
            ...
            for (let redirect = 0; redirect <= OPENAI_FILE_REDIRECT_LIMIT; redirect += 1) {
                try {
                    ...
                    throw new ArtifactError("openai_file_download_failed", "ChatGPT file could not be downloaded.");
                }
                ...
                if (!location || redirect === OPENAI_FILE_REDIRECT_LIMIT) {
                    throw new ArtifactError("openai_file_download_failed", "ChatGPT file download returned an invalid redirect.");
                }
                downloadUrl = validateOpenAIFileUrl(new URL(location, downloadUrl).toString());
            }
            if (!response?.ok || !response.body) {
                await response?.body?.cancel().catch(() => undefined);
                throw new ArtifactError("openai_file_download_failed", "ChatGPT file download did not return file content.");
            }
            ...
            if (reference.size !== undefined
                ...
                await response.body.cancel().catch(() => undefined);
                throw new ArtifactError("openai_file_size_mismatch", "ChatGPT file metadata did not match the downloaded content.");
            }
...
export function describeIncomingArtifactValue(value, maxDepth = 4, maxEntries = 20) {
    const seen = new WeakSet();
    const describe = (current, depth) => {
        if (current === null)
            return { type: "null" };
        ...
        if (typeof current === "number")
            return { type: "number", finite: Number.isFinite(current) };
...
function validateIncomingArtifactSource(source) {
    if (!source || typeof source !== "object") {
        throw new ArtifactError("invalid_incoming_artifact_source", "Incoming artifact adapter returned an invalid source.");
    }
    if (typeof source.name !== "string" || source.name.length === 0) {
        throw new ArtifactError("invalid_incoming_artifact_source", "Incoming artifact adapter must provide a filename.");
    }
    if (source.mimeType !== undefined && typeof source.mimeType !== "string") {
        throw new ArtifactError("invalid_incoming_artifact_source", "Incoming artifact adapter returned an invalid MIME hint.");
    }
    if (source.size !== undefined
        && (!Number.isSafeInteger(source.size) || source.size < 0)) {
        throw new ArtifactError("invalid_incoming_artifact_source", "Incoming artifact adapter returned an invalid byte size.");
    }
    const stream = source.stream;
    if (!stream || typeof stream[Symbol.asyncIterator] !== "function") {
        throw new ArtifactError("invalid_incoming_artifact_source", "Incoming artifact adapter must provide an async-readable stream.");
    }
}
function isOpenAIFileReferenceCandidate(value) {
    if (!isRecord(value))
        return false;
...
function normalizeOpenAIFileReference(value) {
    if (!isOpenAIFileReferenceCandidate(value)) {
        throw new ArtifactError("invalid_openai_file_reference", "ChatGPT file reference is malformed.");
    }
    ...
    if (normalizedFileName
        ...
        throw new ArtifactError("ambiguous_openai_file_name", "ChatGPT file reference contained conflicting filenames.");
    }
    let size;
...
function normalizeOpenAIFileName(suppliedName, fileId, mimeType) {
    if (suppliedName)
        return suppliedName;
...
function validateOpenAIFileUrl(value) {
    let url;
    try {
        url = new URL(value);
    }
    catch {
        throw new ArtifactError("unsafe_openai_file_reference", "ChatGPT file download URL is invalid.");
    }
    if (url.protocol !== "https:"
        ...
        || url.password !== ""
        || url.hash !== "") {
        throw new ArtifactError("unsafe_openai_file_reference", "ChatGPT file download URL is outside the trusted file host.");
    }
    return url.toString();
}
...
function classifyValueString(value) {
...
function safeValueEntryKey(value, index) {
