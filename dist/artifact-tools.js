 ...
 */
export async function downloadIncomingArtifact({ registry, workspaceId, workspaceRoot, maxFileBytes, file, path, publishLink = link, }) {
    if (!isArtifactDownloadSupportedPlatform()) {
        throw new ArtifactError("artifact_platform_unsupported", "Native file download requires descriptor-anchored directory operations on this platform.");
    }
    if (!Number.isSafeInteger(maxFileBytes) || maxFileBytes < 1) {
        throw new ArtifactError("artifact_limit_invalid", "Artifact file-size limit must be a positive integer.");
    }
    if (!workspaceId) {
        throw new ArtifactError("artifact_workspace_invalid", "A selected workspace is required for native file download.");
    }
    ...
    try {
        if (opened.size !== undefined && opened.size > maxFileBytes) {
            throw new ArtifactError("artifact_file_too_large", "Native file exceeds the configured per-file limit.");
        }
        workspaceHandle = await openDirectoryNoFollow(workspaceRoot, "artifact_workspace_unsafe", "Selected workspace root is not a real directory.");
        destinationDirectory = await prepareDestinationDirectory(workspaceHandle, destination.parentParts);
        await cleanupStalePartials(destinationDirectory);
        partialPath = join(destinationDirectory.anchorPath, `${PARTIAL_PREFIX}${randomUUID()}${PARTIAL_SUFFIX}`);
        handle = await open(partialPath, fsConstants.O_WRONLY | fsConstants.O_CREAT | fsConstants.O_EXCL | NO_FOLLOW, 0o600);
        ...
        if (opened.size !== undefined && opened.size !== size) {
            throw new ArtifactError("artifact_file_size_mismatch", "Native file metadata did not match the downloaded content.");
        }
        await handle.sync();
        const writtenEntry = await handle.stat();
        if (!writtenEntry.isFile() || writtenEntry.size !== size) {
            throw new ArtifactError("artifact_write_integrity_failed", "Native file could not be verified before publication.");
        }
        const partialEntry = await lstat(partialPath);
        if (partialEntry.isSymbolicLink()
            || !partialEntry.isFile()
            || partialEntry.dev !== writtenEntry.dev
            || partialEntry.ino !== writtenEntry.ino
            || partialEntry.size !== writtenEntry.size) {
            throw new ArtifactError("artifact_partial_unsafe", "Native file partial changed before publication.");
        }
        await publishDestination(destinationDirectory, partialPath, destination.name, writtenEntry, handle, publishLink);
...
async function assertDirectoryHandle(handle) {
    const entry = await handle.stat();
    if (!entry.isDirectory()) {
        throw new ArtifactError("artifact_directory_unsafe", "Artifact destination parent is not a directory.");
    }
}
...
function normalizeArtifactDestination(value) {
    ...
    if (!value
        ...
        || value.endsWith(sep)
        || rawParts.includes("..")) {
        throw new ArtifactError("artifact_destination_invalid", "Artifact destination must be a non-empty relative file path inside the workspace.");
    }
    const normalized = normalize(value);
    if (normalized === "."
        || normalized === ".."
        || normalized.startsWith(`..${sep}`)) {
        throw new ArtifactError("artifact_destination_invalid", "Artifact destination must stay inside the selected workspace.");
    }
    const parts = normalized.split(sep);
    const name = parts.at(-1);
    if (!name || name === "." || name === "..") {
        throw new ArtifactError("artifact_destination_invalid", "Artifact destination must name a file inside the selected workspace.");
    }
    return {
        path: normalized,
...
async function prepareDestinationDirectory(rootHandle, parentParts) {
    const openedHandles = [];
    let parentHandle = rootHandle;
    let parentAnchor = descriptorDirectoryPath(rootHandle);
    try {
        for (const part of parentParts) {
            const child = await ensureWorkspaceChildDirectory(parentHandle, parentAnchor, part);
...
async function ensureWorkspaceChildDirectory(parentHandle, parentAnchor, name) {
    ...
    return openDirectoryNoFollow(path, "artifact_destination_parent_unsafe", "Artifact destination parent must be a real directory inside the workspace.");
}
async function publishDestination(directory, partialPath, filename, writtenEntry, handle, publishLink) {
    ...
    try {
        ...
        if (isNodeError(error) && error.code === "EEXIST") {
            throw new ArtifactError("artifact_destination_exists", "Artifact destination already exists.");
        }
...
function assertPublishedArtifactEntry(entry, writtenEntry) {
    if (entry.isSymbolicLink()
        || !entry.isFile()
        || entry.dev !== writtenEntry.dev
        || entry.ino !== writtenEntry.ino
        || entry.size !== writtenEntry.size) {
        throw new ArtifactError("artifact_destination_publish_failed", "Published artifact did not match the verified download.");
    }
}
async function cleanupStalePartials(directory) {
    await assertDirectoryHandle(directory.handle);
    const entries = await readdir(directory.anchorPath, { withFileTypes: true });
    let inspected = 0;
    const cutoff = Date.now() - STALE_PARTIAL_AGE_MS;
    for (const entry of entries) {
        if (inspected >= MAX_STALE_PARTIAL_CLEANUP)
            break;
...
function incomingStreamChunk(value) {
    ...
    throw new ArtifactError("invalid_incoming_artifact_chunk", "Incoming artifact stream yielded a value that is not bytes or text.");
}
