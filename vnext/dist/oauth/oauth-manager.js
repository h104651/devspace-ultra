...
function validateRedirectUri(raw, applicationType) {
    let url;
    try {
        url = new URL(raw);
    }
    catch {
        throw new Error('INVALID_CLIENT_METADATA: redirect_uris must contain absolute URIs');
    }
    if (url.username || url.password || url.hash)
        throw new Error('INVALID_CLIENT_METADATA: redirect URI must not contain credentials or fragment');
    if (applicationType === 'web') {
        if (url.protocol !== 'https:')
            throw new Error('INVALID_CLIENT_METADATA: web redirect URIs must use https');
    }
    else if (url.protocol === 'http:') {
        if (!isLoopbackHostname(url.hostname))
            throw new Error('INVALID_CLIENT_METADATA: native http redirect URIs must be loopback only');
    }
...
class OAuthManager {
    ...
    issuerUrl;
    expectedResource;
    constructor(issuerUrl, authManager, storage) {
    ...
    getAuthorizationServerMetadata() {
        return {
            ...
            scopes_supported: exports.CHATGPT_LEAST_PRIVILEGE_SCOPES,
    ...
    async registerClient(body) {
        ...
        const redirectUris = Array.isArray(body?.redirect_uris) ? body.redirect_uris.filter((v) => typeof v === 'string') : [];
        if (redirectUris.length === 0)
            throw new Error('INVALID_CLIENT_METADATA: redirect_uris is required');
        ...
        if (tokenAuthMethod !== 'none')
            throw new Error('INVALID_CLIENT_METADATA: only public PKCE clients (token_endpoint_auth_method=none) are supported');
        if (!grantTypes.includes('authorization_code') || grantTypes.some((g) => !['authorization_code', 'refresh_token'].includes(g))) {
            throw new Error('INVALID_CLIENT_METADATA: unsupported grant_types');
        }
        if (responseTypes.length !== 1 || responseTypes[0] !== 'code')
            throw new Error('INVALID_CLIENT_METADATA: response_types must be ["code"]');
        const clientId = `chatgpt_client_${crypto_1.default.randomUUID()}`;
        const record = {
            clientId,
            clientName,
            redirectUris,
    ...
    async createAuthorizationCode(params) {
        ...
        if (!client)
            throw new Error('INVALID_REQUEST: unknown client_id');
        if (!params.redirectUri || !client.redirectUris.includes(params.redirectUri))
            throw new Error('INVALID_REQUEST: redirect_uri is not registered for this client');
        if (!params.codeChallenge || params.codeChallengeMethod !== 'S256')
            throw new Error('INVALID_REQUEST: PKCE S256 code_challenge is required');
    ...
    async exchangeCodeForTokens(params) {
        if (!params.code || !params.clientId || !params.redirectUri || !params.codeVerifier)
            throw new Error('INVALID_REQUEST: code, client_id, redirect_uri and code_verifier are required');
        ...
        if (!client || !client.redirectUris.includes(params.redirectUri))
            throw new Error('INVALID_GRANT: client registration is invalid');
        const requestedResource = params.resource || codeRecord.resource;
        if (requestedResource !== codeRecord.resource || codeRecord.resource !== this.expectedResource)
            throw new Error('INVALID_TARGET: resource mismatch');
        const computed = crypto_1.default.createHash('sha256').update(params.codeVerifier).digest('base64url');
        if (computed !== codeRecord.codeChallenge)
            throw new Error('INVALID_GRANT: PKCE code_verifier challenge mismatch');
        await this.storage.deleteOAuthCode(params.code);
        const scopes = codeRecord.scope.split(' ').filter((s) => exports.CHATGPT_LEAST_PRIVILEGE_SCOPES.includes(s));
    ...
    async refreshAccessToken(refreshToken, requestedResource) {
        ...
        if (val.payload.metadata?.purpose !== 'refresh_token')
            throw new Error('INVALID_GRANT: token is not a refresh token');
        if (await this.storage.isTokenRevoked(val.payload.tokenId))
        ...
        if (boundResource !== this.expectedResource || (requestedResource && requestedResource !== boundResource))
            throw new Error('INVALID_TARGET: resource mismatch');
        const granted = (val.payload.scopes || []).filter(s => exports.CHATGPT_LEAST_PRIVILEGE_SCOPES.includes(s));
    ...
    buildAuthorizationRedirect(redirectUri, code, state) {
    ...
    renderAuthorizationPage(params) {
        ...
        const hidden = (name, value) => `<input type="hidden" name="${name}" value="${htmlEscape(value)}">`;
        return `<!DOCTYPE html>
