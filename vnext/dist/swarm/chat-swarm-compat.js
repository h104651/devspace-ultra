...
exports.DurableChatSwarmCompat = void 0;
...
class DurableChatSwarmCompat {
    ...
    async load() {
        ...
        return stored?.version === 1 && stored?.swarms ? stored : emptyState();
    }
    async save(state) {
    ...
    async findSwarmByInvite(state, inviteCode) {
    ...
    async findOrchestrator(state, token) {
    ...
    async findWorker(state, token) {
        const hash = await sha256(token);
        for (const swarm of Object.values(state.swarms)) {
            for (const worker of Object.values(swarm.workers)) {
                if (worker.active && worker.tokenHash === hash)
                    return { swarm, worker };
            }
        }
        throw new Error('Invalid or inactive worker token.');
    }
    async findBrowserWorker(state, token) {
        const hash = await sha256(token);
        for (const swarm of Object.values(state.swarms)) {
            for (const worker of Object.values(swarm.workers)) {
                if (worker.active && worker.browserWakeTokenHash === hash)
                    return { swarm, worker };
            }
        }
    ...
    async status(token) {
        return this.mutate(async (state) => {
            ...
            if (orchestrator)
                return this.summary(orchestrator, 'orchestrator');
    ...
    async workerEvent(workerToken) {
        return this.mutate(async (state) => {
            ...
            worker.lastSeenAt = worker.dockLastSeenAt;
            if (swarm.state !== 'active')
                return { type: 'closed', swarmId: swarm.id, workerId: worker.id };
            if (worker.inFlightTaskId) {
                const inFlight = swarm.tasks[worker.inFlightTaskId];
                if (inFlight?.status === 'claimed' && inFlight.workerId === worker.id) {
            ...
            if (!task)
                return { type: 'parked', swarmId: swarm.id, workerId: worker.id };
            return {
                type: 'task_available',
                swarmId: swarm.id,
                workerId: worker.id,
    ...
    async resize(input) {
        return this.mutate(async (state) => {
            ...
            for (const task of Object.values(swarm.tasks)) {
                if (['queued', 'claimed'].includes(task.status) && task.targetWorkerId)
                    protectedIds.add(task.targetWorkerId);
            }
    ...
    async dispatch(input) {
        ...
        return this.mutate(async (state) => {
            ...
            for (const spec of input.tasks) {
                if (!spec?.prompt || typeof spec.prompt !== 'string')
                    throw new Error('Each task requires a prompt.');
                if (spec.prompt.length > MAX_PROMPT_CHARS)
                    throw new Error(`Task prompt exceeds ${MAX_PROMPT_CHARS} characters.`);
                if (spec.taskKey && seenKeys.has(spec.taskKey))
                    throw new Error(`Duplicate taskKey '${spec.taskKey}' in dispatch batch.`);
                if (spec.taskKey)
                    seenKeys.add(spec.taskKey);
            ...
            for (const spec of input.tasks) {
                ...
                if (spec.taskKey)
                    swarm.taskKeys[spec.taskKey] = taskId;
    ...
    async acknowledge(input) {
        return this.mutate(async (state) => {
            ...
            if (task.workerId !== worker.id || task.status !== 'claimed')
                throw new Error(`Task ${input.taskId} is not claimed by ${worker.id}.`);
    ...
    async submit(input) {
        const submitted = await this.mutate(async (state) => {
            ...
            if (task.status === 'cancelled')
                throw new Error(`Task ${input.taskId} was cancelled and cannot accept a result.`);
            if (task.status === 'completed' || task.status === 'failed') {
                if (task.workerId !== worker.id)
                    throw new Error(`Task ${input.taskId} was already completed by another worker.`);
            ...
            if (resultText.length > MAX_RESULT_CHARS)
                throw new Error(`Result exceeds ${MAX_RESULT_CHARS} characters.`);
    ...
    async recycleWorker(input) {
        return this.mutate(async (state) => {
            ...
            if (!worker.active)
                return { ok: true, swarmId: swarm.id, workerId: worker.id, state: 'recycled', duplicate: true };
            let requeuedTask;
            if (worker.inFlightTaskId) {
                const task = swarm.tasks[worker.inFlightTaskId];
                if (task?.status === 'claimed' && task.workerId === worker.id) {
    ...
    async bindBrowserByInvite(inviteCode) {
        const browserWakeToken = randomToken();
        return this.mutate(async (state) => {
    ...
    async browserClaim(browserWakeToken) {
        return this.mutate(async (state) => {
            const { swarm, worker } = await this.findBrowserWorker(state, browserWakeToken);
    ...
    async browserEvent(browserWakeToken) {
        return this.mutate(async (state) => {
            const { swarm, worker } = await this.findBrowserWorker(state, browserWakeToken);
            worker.browserOnline = true;
            worker.browserLastSeenAt = nowIso();
...
exports.DurableChatSwarmCompat = DurableChatSwarmCompat;
