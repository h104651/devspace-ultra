...
export class GatewayDurableObject {
  ...
  constructor(private ctx: any, private env: Env) {
    ...
    this.killSwitch = new KillSwitch(this.storage);
  ...
  private authenticateAdmin(request: Request): Response | undefined {
    const adminSecret = this.env.ADMIN_SECRET;
    if (!adminSecret || adminSecret.length < 16) {
      return Response.json(
        { error: 'ADMIN_NOT_CONFIGURED', message: 'ADMIN_SECRET is not configured on this gateway' },
        { status: 503, headers: this.corsHeaders() }
      );
    }
    ...
    if (!candidate) {
      return Response.json(
        { error: 'AUTH_REQUIRED', message: 'Admin credential required (X-Admin-Secret header)' },
    ...
    if (!this.timingSafeEqualStr(candidate, adminSecret)) {
      return Response.json(
        { error: 'AUTH_INVALID', message: 'Invalid admin credential' },
        { status: 401, headers: this.corsHeaders() }
      );
    }
  ...
  private async processSingleMcpItem(body: any, auth: TokenPayload, modern: boolean, protocolVersion: string): Promise<Response> {
    ...
    const wrapList = <T extends Record<string, any>>(value: T) => modern
      ? modernCacheableResult(value, { ttlMs: 0, cacheScope: 'private' })
      : value;
