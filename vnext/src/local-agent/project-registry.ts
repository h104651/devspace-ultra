...
export class ProjectPathSecurity {
   ...
   */
  static resolveDirectoryPath(canonicalRoot: string, relativePath?: string): string {
    const safeRel = this.validateRelativePath(relativePath || '.');
    const resolved = path.resolve(canonicalRoot, safeRel);
    ...
    if (!fs.existsSync(resolved)) {
      const err: any = new Error(`LOCAL_DIRECTORY_NOT_FOUND: Directory '${relativePath || '.'}' does not exist in project`);
    ...
    if (!this.isPathInsideRoot(realTarget, canonicalRoot)) {
      const err: any = new Error(`LOCAL_PROJECT_PATH_ESCAPE: Target directory '${relativePath || '.'}' resolves via symlink/junction outside project root to '${realTarget}'`);
      err.code = 'LOCAL_PROJECT_PATH_ESCAPE';
      throw err;
    }
    ...
    if (!stat.isDirectory()) {
      const err: any = new Error(`LOCAL_NOT_A_DIRECTORY: Path '${relativePath || '.'}' is not a directory`);
