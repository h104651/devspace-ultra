...
export class McpHandlers {
  ...
  private mockWorkspaces: Map<string, { manifest: DevSpaceProjectManifest; files: Record<string, string> }> = new Map();

  private getOrCreateWorkspace(projectRef: string, owner: string, slug: string) {
    if (!this.mockWorkspaces.has(projectRef)) {
      const isAstor = projectRef.includes('astor') || slug.includes('astor');
      const manifest: DevSpaceProjectManifest = {
  ...
  public async handleKaggleWorkspaceGet(args: any, caller?: McpCallerContext) {
    ...
    const fingerprint = computeWorkspaceFingerprint(ws.manifest);
  ...
  public async handleKaggleWorkspaceContinue(args: any, caller?: McpCallerContext) {
    ...
    const ws = this.getOrCreateWorkspace(ref, owner, slug);
    const currentFingerprint = computeWorkspaceFingerprint(ws.manifest);
    ...
    // Apply file changes
    const uploadedFiles: { token: string; description?: string }[] = [];
    ...
    const datasetResult = await client.createDatasetVersion(
      slug,
      `DevSpace Workspace Version ${ws.manifest.version}: ${args.reason}`,
      uploadedFiles
    );
    ...
    const newFingerprint = computeWorkspaceFingerprint(ws.manifest);
