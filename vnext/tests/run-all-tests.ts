...
async function main() {
  ...
  const suites: Array<{ name: string; runner: () => Promise<{ passed: number; failed: number }> }> = [
    ...
    { name: 'Cloudflare: OAuth 2.0 & Protected Resource Metadata', runner: runOAuthTests },
  ...
  for (const suite of suites) {
    ...
    try {
      ...
      if (res.failed === 0) {
        console.log(`\x1b[32mPASS\x1b[0m (${res.passed} tests, ${duration}ms)`);
      } else {
        console.log(`\x1b[31mFAIL\x1b[0m (${res.passed} passed, ${res.failed} failed, ${duration}ms)`);
      }
    } catch (err: any) {
      totalFailed++;
